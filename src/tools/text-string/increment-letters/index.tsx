...
function IncrementLetters() {
  ...
  useEffect(() => {
    ...
    const incrementLetter = (char: string, shiftAmount: number): string => {
    ...
    const result = inputText
      .split('')
      .map(char => incrementLetter(char, shift))
      .join('');
  ...
  const copyToClipboard = async () => {
    ...
    try {
      ...
      toast({
        title: "Copied!",
        description: "Incremented text copied to clipboard",
      });
    } catch (err) {
      toast({
      ...
      <Card className="border-2 shadow-lg">
        <CardHeader className="text-center space-y-2 pb-4 bg-gradient-to-r from-slate-50 to-gray-50 dark:from-slate-950/20 dark:to-gray-950/20">
          <div className="flex items-center justify-center gap-2">
            ...
            <CardTitle className="text-2xl">Increment Letters in Text</CardTitle>
          </div>
          <CardDescription>Shift letters forward in the alphabet (Caesar cipher)</CardDescription>
        </CardHeader>
        <CardContent className="pt-6 space-y-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Enter a number (e.g., 1 shifts A→B, 2 shifts A→C)
            </p>
          </div>
          ...
          <div>
            <Label htmlFor="output">Output (Incremented):</Label>
            <Textarea
              id="output"
              ...
              placeholder="Incremented text will appear here..."
            />
          </div>

          <div className="bg-slate-50 dark:bg-slate-950/20 p-3 rounded-lg">
            <p className="text-sm text-slate-700 dark:text-slate-300">
              <strong>Example:</strong> "ABC" with shift 1 → "BCD" (each letter shifted forward)
            </p>
          </div>
