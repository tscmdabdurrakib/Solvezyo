...
function RemoveAllWhitespace() {
  const [inputText, setInputText] = useState<string>('Remove all whitespace from this text');
      ...
      <Card className="border-2 shadow-lg">
        <CardHeader className="text-center space-y-2 pb-4 bg-gradient-to-r from-slate-50 to-gray-50 dark:from-slate-950/20 dark:to-gray-950/20">
          <div className="flex items-center justify-center gap-2">
            <Minimize2 className="h-6 w-6 text-slate-600 dark:text-slate-400" />
            <CardTitle className="text-2xl">Remove All Whitespace</CardTitle>
          </div>
          <CardDescription>Remove all spaces, tabs, and line breaks from your text</CardDescription>
        </CardHeader>
        <CardContent className="pt-6 space-y-4">
          <div>
            <Label htmlFor="input-text">Input Text (With Whitespace):</Label>
            <Textarea
              id="input-text"
          ...
          <div>
            <Label htmlFor="output">Output (No Whitespace):</Label>
            <Textarea
              id="output"
          ...
          <div className="bg-slate-50 dark:bg-slate-950/20 p-3 rounded-lg">
            <p className="text-sm text-slate-700 dark:text-slate-300">
              <strong>Example:</strong> "Hello World" → "HelloWorld" (all whitespace removed)
            </p>
          </div>
