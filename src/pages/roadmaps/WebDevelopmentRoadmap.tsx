...
const WebDevelopmentRoadmap: React.FC = () => {
  const { toast } = useToast();
  const [bookmarked, setBookmarked] = React.useState(false);
  
  // Get the specific roadmap data
  const roadmapDetail = roadmapDetails['web-development'];
  
  const handleBookmark = () => {
    setBookmarked(!bookmarked);
    toast({
      title: bookmarked ? "Removed from bookmarks" : "Added to bookmarks",
      description: `Web Development Roadmap ${bookmarked ? "removed from" : "added to"} your bookmarks.`,
    });
  };
        ...
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-6">
          <div>
            <div className="flex items-center mb-4">
              <Code className="h-10 w-10 text-primary mr-3" />
