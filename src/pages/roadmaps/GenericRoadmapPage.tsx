...
const GenericRoadmapPage: React.FC = () => {
  ...
  // Find the roadmap details by ID
  const roadmapDetail = roadmapDetails[id];
  
  if (!roadmap || !roadmapDetail) {
    return (
  ...
  const handleBookmark = () => {
    ...
    toast({
      ...
      description: `${roadmap.title} ${bookmarked ? "removed from" : "added to"} your bookmarks.`,
    });
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-6">
          <div>
            <div className="flex items-center mb-4">
              {Icon && <Icon className="h-10 w-10 text-primary mr-3" />}
      ...
      <div className="mb-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {roadmaps
            .filter(r => r.id !== id) // Exclude current roadmap
            .slice(0, 3) // Take first 3
            .map((relatedRoadmap) => {
            const RelatedIcon = relatedRoadmap.icon;
            return (
              <Card key={relatedRoadmap.id}>
                <CardHeader>
                  <div className="flex items-center">
                    {RelatedIcon && <RelatedIcon className="h-6 w-6 text-primary mr-2" />}
                    <CardTitle className="text-lg">{relatedRoadmap.title}</CardTitle>
                  </div>
                </CardHeader>
                <CardContent>
                  ...
                  <Link to={`/roadmaps/${relatedRoadmap.id}`}>
                    <Button variant="outline" size="sm">
                      View Roadmap
                    </Button>
                  </Link>
...
export default GenericRoadmapPage;